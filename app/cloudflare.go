...
func CloudFlare(c cfg.Config, cfCoins []map[string]interface{}) {
...
func createDNS(cfCoins []map[string]interface{}, api *cf.API, ctx context.Context, domain string) {
	// Fetch the zone ID
	...
	for _, cfCoin := range cfCoins {
		//_, err := http.Get("https://" + slug + "." + domain)
		//if err != nil {
		setDNS(api, ctx, registrated, domain, cfCoin["slug"].(string))
	}
}
